...
-export([body/1, body_qs/1, callback/1, header/2,
	 jsessionid/1, method/1, path/1, peername/1,
	 sockname/1]).

-export([chunk/2, chunk_end/1, chunk_start/3, reply/4]).

-export([abruptly_kill/1, hook_tcp_close/1,
	 unhook_tcp_close/1]).
...
path({cowboy, Req}) ->
    Path = cowboy_req:path(Req), binary_to_list(Path).

-spec method(req()) -> atom().

method({cowboy, Req}) ->
    Method = cowboy_req:method(Req), method_atom(Method).
...
body_qs(Req) ->
    ...
    case H of
      H when H =:= "text/plain" orelse H =:= "" -> body(Req1);
      _ ->
...
-spec header(atom(), req()) -> {nonempty_string() |
				undefined,
				req()}.
...
-spec jsessionid(req()) -> {nonempty_string() |
			    undefined,
			    req()}.

jsessionid({cowboy, Req}) ->
    #{'JSESSIONID' := C} = cowboy_req:cookie([{'JSESSIONID',
					       [], undefined}],
					     Req),
    case C of
...
-spec callback(req()) -> {nonempty_string() | undefined,
			  req()}.
...
-spec peername(req()) -> {inet:ip_address(),
			  non_neg_integer()}.

peername({cowboy, Req}) -> cowboy_req:peer(Req).

-spec sockname(req()) -> {inet:ip_address(),
			  non_neg_integer()}.
...
-spec reply(non_neg_integer(), headers(), iodata(),
	    req()) -> req().

reply(Code, Headers, Body, {cowboy, Req}) ->
    Body1 = iolist_to_binary(Body),
    {ok, Req1} = cowboy_req:reply(Code, enbinary(Headers),
				  Body1, Req),
    {cowboy, Req1}.

-spec chunk_start(non_neg_integer(), headers(),
		  req()) -> req().

chunk_start(Code, Headers, {cowboy, Req}) ->
    {ok, Req1} = cowboy_req:chunked_reply(Code,
					  enbinary(Headers), Req),
    {cowboy, Req1}.
...
chunk(Chunk, {cowboy, Req} = R) ->
    case cowboy_req:chunk(Chunk, Req) of
      ...
      {error, _E} ->
	  {error,
	   R}                      %% This shouldn't happen too often, usually we
