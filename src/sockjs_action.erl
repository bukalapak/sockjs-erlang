...
% recv
-export([jsonp_send/4, xhr_send/4]).

% misc
-export([rawwebsocket/3, websocket/3]).
...
-define(IFRAME,
    "<!DOCTYPE html>\n<html>\n<head>\n  <meta "
    "http-equiv=\"X-UA-Compatible\" content=\"IE=e"
    "dge\" />\n  <meta http-equiv=\"Content-Type\" "
    "content=\"text/html; charset=UTF-8\" "
    "/>\n  <script>\n    document.domain "
    "= document.domain;\n    _sockjs_onload "
    "= function(){SockJS.bootstrap_iframe();};\n "
    " </script>\n  <script src=\"~s\"></script>\n<"
    "/head>\n<body>\n  <h2>Don't panic!</h2>\n "
    " <p>This is a SockJS hidden iframe. "
    "It's used for cross domain magic.</p>\n</body"
    ">\n</html>"
).
...
xhr_streaming(
    Req,
    Headers,
    Service = #service{response_limit = ResponseLimit},
    Session
) ->
    Req1 = chunk_start(Req, Headers),
    ...
    Req2 = chunk(
        Req1,
        list_to_binary(string:copies("h", 2048)),
        fun fmt_xhr/1
    ),
    reply_loop(
        Req2,
...
htmlfile(
    ...
    S = fun(Req1, CB) ->
        ...
        IFrame =
            iolist_to_binary(io_lib:format(?IFRAME_HTMLFILE, [CB])),
            ...
            receive
                ...
                {tcp, _S, Data} ->
                    error_logger:error_msg(
                        "Received unexpected data on a long-polling "
                        "http connection: ~p. Connection aborted.~n",
                        [Data]
                    ),
                ...
                go ->
                    ...
                    reply_loop(Req1, SessionId, ResponseLimit, Fmt, Service)
            end;
        session_in_use ->
            Frame = sockjs_util:encode_frame({close, ?STILL_OPEN}),
...
chunk_end(Req, Body, Fmt) ->
    Req1 = chunk(Req, Body, Fmt),
    chunk_end(Req1).
...
fmt_jsonp(Body, Callback) ->
    ...
    [
        Callback,
        "(",
        sockjs_json:encode(iolist_to_binary(Body)),
        ");\r\n"
    ].
...
websocket(Req, Headers, Service) ->
    {_Any, Req1, {R1, R2}} =
        sockjs_handler:is_valid_ws(Service, Req),
    case {R1, R2} of
        {false, _} ->
            sockjs_http:reply(
                400,
        ...
        {true, true} ->
            sockjs_http:reply(
                400,
                Headers,
                "This WebSocket request can't be handled.",
                Req1
            )
    end.
