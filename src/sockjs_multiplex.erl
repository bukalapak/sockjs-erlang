...
init_state(Services, {AuthenCallback, Options}) ->
    L = [
        {Topic, #service{callback = Callback, state = State}}
        || {Topic, Callback, State} <- Services
    ],
    Extra =
        case lists:keyfind(state, 1, Options) of
            {state, ExtraValue} ->
...
get_authen_callback_result(
    ...
    case erlang:is_function(AuthenCallback) of
        true -> AuthenCallback(Handle, What, Extra);
        false -> authen_callback_not_found
    end.

sockjs_init(
    Conn,
    {Services, Channels, AuthenCallbackRec, Extra} = S
) ->
    case
        get_authen_callback_result(
            AuthenCallbackRec,
    ...
    of
        authen_callback_not_found -> {ok, S};
        {ok, Extra1} -> {ok, {Services, Channels, AuthenCallbackRec, Extra1}}
    end.

sockjs_handle_via_channel(
    Conn,
    Data,
    {Services, Channels, AuthenCallbackRec, Extra}
) ->
    [Type, Topic, Payload] = split(
        $,,
        binary_to_list(Data),
        3
    ),
...
sockjs_handle(
    Conn,
    Data,
    {Services, Channels, #authen_callback{success = Success} = AuthenCallbackRec, Extra} =
        S
) ->
    case Success of
        true ->
        ...
        false ->
            ...
            of
                ...
                {success, Extra1} ->
                    {ok,
                        {Services, Channels, AuthenCallbackRec#authen_callback{success = true},
                            Extra1}};
                {ok, Extra1} ->
