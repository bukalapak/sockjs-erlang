...
-spec init_state(binary(), callback(), any(),
		 [tuple()]) -> service().
...
init_state(Prefix, Callback, State, Options) ->
    #service{prefix = binary_to_list(Prefix),
	     ...
	     response_limit =
		 proplists:get_value(response_limit, Options,
				     128 * 1024),
	     logger =
		 proplists:get_value(logger, Options,
...
-spec is_valid_ws(service(), req()) -> {boolean(),
					tuple()}.
...
-spec valid_ws_request(service(), req()) -> {boolean(),
					     tuple()}.
...
-spec get_action(service(), req()) -> nomatch |
				      {match, atom()}.
...
strip_prefix(LongPath, Prefix) ->
    ...
    case Prefix of
      ...
      _Any ->
	  {error,
	   io_lib:format("Wrong prefix: ~p is not ~p",
			 [A, Prefix])}
    end.

-type dispatch_result() :: nomatch |
			   {match,
			    {send | recv | none, atom(), server(), session(),
			     [atom()]}} |
			   {bad_method, [atom()]}.

-spec dispatch_req(service(),
		   req()) -> dispatch_result().
...
-spec dispatch(atom(),
	       nonempty_string()) -> dispatch_result().

dispatch(Method, Path) ->
    lists:foldl(fun ({Match, MethodFilters}, nomatch) ->
			case Match(Path) of
			  nomatch -> nomatch;
			  [Server, Session] ->
...
filters() ->
	...
	[h_sid, xhr_cors, cache_for, xhr_options_get]}]}].
...
re(Path, S) ->
    case re:run(Path, S, [{capture, all_but_first, list}])
	of
      nomatch -> nomatch;
      {match, []} -> [dummy, dummy];
...
handle({bad_method, Methods}, _Service, Req) ->
    MethodsStr = string:join([atom_to_list(M)
			      || M <- Methods],
			     ", "),
    H = [{"Allow", MethodsStr}],
    sockjs_http:reply(405, H, "", Req);
handle({match,
	{Type, Action, _Server, Session, Filters}},
       Service, Req) ->
    {Headers, Req2} = lists:foldl(fun (Filter,
				       {Headers0, Req1}) ->
    ...
    case Type of
      ...
      recv ->
	  try sockjs_action:Action(Req2, Headers, Service,
				   Session)
	  catch
	    no_session ->
      ...
      none -> sockjs_action:Action(Req2, Headers, Service)
    end.

%% --------------------------------------------------------------------------

-spec default_logger(service(), req(),
		     websocket | http) -> no_return().
...
extract_info(Req) ->
    ...
    [{peername, Peer}, {sockname, Sock}, {path, Path},
     {headers, Headers}].
