...
-export([decode/1, decode/2, decoder/1]).
...
decode(S, Options) ->
    json_decode(
        S,
        parse_decoder_options(Options, #decoder{})
    ).
...
parse_encoder_options([{utf8, Switch} | Rest], State) ->
    parse_encoder_options(
        Rest,
        State#encoder{utf8 = Switch}
    ).

parse_decoder_options([], State) ->
    State;
parse_decoder_options(
    [{object_hook, Hook} | Rest],
    State
) ->
    parse_decoder_options(
        Rest,
...
parse_decoder_options([{format, Format} | Rest], State) when
    Format =:= struct orelse
        Format =:= eep18 orelse Format =:= proplist
->
    parse_decoder_options(
        Rest,
...
json_encode([{K, _} | _] = Props, State) when
    K =/= struct andalso
...
json_encode_string(L, State) when is_list(L) ->
    case json_string_is_safe(L) of
        true -> [?Q, L, ?Q];
        false -> json_encode_string_unicode(L, State, [?Q])
    end.
...
json_string_is_safe([C | Rest]) ->
    case C of
        ...
        $\r -> false;
        $\t -> false;
        C when C >= 0, C < $\s; C >= 127, C =< 1114111 -> false;
        C when C < 127 -> json_string_is_safe(Rest);
        _ -> false
    end.
...
json_bin_is_safe(<<C, Rest/binary>>) ->
    case C of
        ...
        $\r -> false;
        $\t -> false;
        C when C >= 0, C < $\s; C >= 127 -> false;
        C when C < 127 -> json_bin_is_safe(Rest)
    end.
...
json_encode_string_unicode([C | Cs], State, Acc) ->
    Acc1 =
        case C of
            ...
            C when C >= 0, C < $\s ->
                [unihex(C) | Acc];
            C when C >= 127, C =< 1114111, State#encoder.utf8 ->
                [xmerl_ucs:to_utf8(C) | Acc];
            C when C >= 127, C =< 1114111, not State#encoder.utf8 ->
                [unihex(C) | Acc];
            C when C < 127 ->
                [C | Acc];
            _ ->
...
decode1(B, S = #decoder{state = null}) ->
    case tokenize(B, S#decoder{state = any}) of
        {{const, C}, S1} -> {C, S1};
        {start_array, S1} -> decode_array(B, S1);
        {start_object, S1} -> decode_object(B, S1)
    end.
...
make_object(
    {struct, P},
    #decoder{object_hook = eep18}
) ->
    {P};
make_object(
    {struct, P},
    #decoder{object_hook = proplist}
) ->
    P;
...
tokenize_string(B, S = #decoder{offset = O}) ->
    case tokenize_string_fast(B, O) of
        ...
        O1 ->
            Length = O1 - O,
            <<_:O/binary, String:Length/binary, (?Q), _/binary>> =
                B,
                ...
                C > 55295, C < 56320 ->
                    ...
                    [CodePoint] =
                        xmerl_ucs:from_utf16be(
                            <<C:16/big-unsigned-integer, D:16/big-unsigned-integer>>
                        ),
                    Acc1 = lists:reverse(xmerl_ucs:to_utf8(CodePoint), Acc),
                    tokenize_string(B, ?ADV_COL(S, 12), Acc1);
                true ->
...
tokenize_number(B, S) ->
    case tokenize_number(B, sign, S, []) of
        {{int, Int}, S1} -> {{const, list_to_integer(Int)}, S1};
        {{float, Float}, S1} -> {{const, list_to_float(Float)}, S1}
    end.
